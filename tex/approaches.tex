...
We then quantitatively evaluate these approaches in Sec.~\ref{sec:evaluation}, shedding further light on the current state of ML-based Android malware detection.
...
In Sec~\ref{sec:systematic_investigation} we have presented recent advancements in ML-based Android malware detection, by dissecting the detection pipeline into three phases: APK characterization, feature representation, and ML modeling.
...
We also ensure that the selected approaches cover almost all the APK characterization, feature representation, and ML models discussed in Sec.~\ref{sec:systematic_investigation}.
...
MamaDroid~\cite{mariconti2016mamadroid} pioneers the use of Markov chains to depict apps' behaviors.
...
% By constructing a Markov chain based on the sequence of abstracted API calls invoked by an app, the transition probabilities between these calls are computed as features. 
\jh{These features then inform an RF model to detect Android malware.}
...
It then captures apps' semantics with meta-path techniques [89].
...
% Building on this, we emphasize the importance of employing a unified framework to evaluate the effectiveness, robustness, and efficiency of ML-based Android malware detection techniques.
...
% However, the experimental setup varies dramatically across these approaches, as shown in Table~\ref{tab:comparative_study}, making direct comparisons challenging.
% As indicated in Table~\ref{tab:comparative_study}, the experimental setup demonstrates large variations across studies, making direct comparisons challenging.
...
To measure a new technique, the importance of efficiency stands parallel to effectiveness.
...
\bulletpoint{Other considerations}
Table~\ref{tab:comparative_study} additionally provides information on whether the selected approaches make their artifacts available and the specific toolchains they utilize.
We note that nearly half of them have not released their artifacts, posing significant challenges for reproducibility and further research.
Furthermore, the toolchains used by them are diverse, such as Androguard~\cite{androguard}, APKTool~\cite{apkt}, and BackSmali~\cite{backsmali}.
...
% Combining with the aforementioned analysis, there is a pressing need for a general-purpose framework that not only unifies the development process but also supports various evaluation scenarios for ML-based Android malware detectors.
